...
# funzione per la rimozione di un pacco consegnato riferito ad un certo ordine
def removeBox(boxID):
...
#thread in ascolto per la remove box
...
def euc_dist_3(drone_pos, dest_pos):
    return math.sqrt(math.pow((drone_pos[0] - dest_pos[0]), 2) + math.pow((drone_pos[1] - dest_pos[1]), 2) + math.pow(
        (drone_pos[2] - dest_pos[2]), 2))
...
# funzione di creazione di un nuovo ordine e posizionamento della box in una base di deposito
def createOrder(orderID, weight, position, base):
    root_node = supervisor.getRoot()
    children_field = root_node.getField("children")
    message = struct.pack("ciidddddd", b"N", int(orderID), int(base), float(weight), float(position[0]),
                          float(position[1]), 0.0, 0.0, 0.0)
    dPrint("New pickup ID: " + str(orderID) + " position: [ " + str(position[0]) + " , " + str(
        position[1]) + " ] base: " + str(base))
