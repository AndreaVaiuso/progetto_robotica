...
# funzione per il calcolo del punteggio per l'attribuzione di un ordine
def sccal(orders, pending, current, state_history, posit, base_coords):
    ...
    while len(orders_new) > 0:
        ...
        if order[2] != -1:
            ...
            distanza_consegna = euc_dist([order[6], order[7]],
                                         [order[4], order[5]])  # dalla posizione del pacco alla consegna
    ...
    if current == []:
        ...
        if "go_back_home" in state_history:
            ...
            distanza_consegna = euc_dist([posit.x, posit.y],
                                         [current[4], current[5]])  # dalla posizione del drone alla consegna
            distanza_ritorno = euc_dist([s_x, s_y], [current[4], current[5]])  # dalla consegna alla base
            tempo_percorso = tempo_percorso + distanza_consegna * 1.7 + land_on_delivery_to_go_back_home + distanza_ritorno * 1.7 + (
                    ostacoli / 1.5)
            ...
            if current[2] != -1:
                ...
                distanza_consegna = euc_dist([current[6], current[7]],
                                             [current[4], current[5]])  # dalla posizione del pacco alla consegna
                distanza_ritorno = euc_dist([s_x, s_y], [current[4], current[5]])  # dalla consegna alla base
                distanza = distanza_pacco + distanza_consegna + distanza_ritorno
                tempo_percorso = tempo_percorso + distanza * 1.7 + reach_quote_to_lock_box + land_on_delivery_to_go_back_home + ostacoli
